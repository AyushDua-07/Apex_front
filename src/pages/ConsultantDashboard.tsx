...
import { Calendar, Clock, Edit3, Save, Plus, Trash2, Shield, AlertCircle, CheckCircle, XCircle, Video, Star } from 'lucide-react';
import { getMyConsultantProfile, updateMyConsultantProfile, getMyAvailability, addMyAvailability, deleteMyAvailability, getMyAppointments, getMyPlanUsage } from '../services/api';
import type { Consultant, Availability, Appointment } from '../types';
import { formatCurrency, formatDate, formatTime, getStatusColor, getInitials } from '../utils/helpers';
...
const ConsultantDashboard: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // Load plan usage (non-blocking — don't fail the whole page if this errors)
      try {
        const uRes = await getMyPlanUsage();
        setPlanUsage(uRes.data);
      } catch {
        // Plan usage is optional info, don't block dashboard
      }
    } catch {
  ...
  const statusMessage = profile?.verificationStatus === 'approved'
    ...
    : profile?.verificationStatus === 'rejected'
    ? 'Your profile has been rejected. Please update your details and contact support.'
    : 'Your profile is pending admin review. You will be visible to clients once approved.';
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              {!editing ? (
                ...
                <div className="flex gap-2">
                  <button onClick={() => setEditing(false)} className="px-3 py-1 text-sm text-gray-600 border border-gray-300 rounded-lg hover:bg-gray-50">
                    Cancel
                  </button>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl shadow-md p-6">
            ...
            {profile?.verificationStatus !== 'approved' && (
              <p className="text-sm text-gray-500 mb-4">
                You can manage availability once your profile is approved.
              </p>
            )}
            ...
            {showAddSlots && profile?.verificationStatus === 'approved' && (
              <div className="bg-gray-50 rounded-xl p-4 mb-4 space-y-3">
                ...
                <button onClick={handleAddSlot} className="w-full py-2 bg-royal text-white text-sm font-medium rounded-lg hover:bg-royal-dark">
                  Add Slot
                </button>
              </div>
            )}
                      ...
                      {dateSlots.map((slot) => (
                        <div key={slot._id} className={`flex items-center justify-between px-3 py-2 rounded-lg text-sm ${slot.isBooked ? 'bg-red-50 text-red-700' : 'bg-gray-50 text-gray-700'}`}>
                          ...
                          {slot.isBooked ? (
                            ...
                            <button onClick={() => handleDeleteSlot(slot._id)} className="text-gray-400 hover:text-red-500">
                              <Trash2 size={14} />
                            </button>
                          )}
                        </div>
          ...
          <div className="bg-white rounded-2xl shadow-md p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/pricing" className="text-royal text-xs font-medium hover:underline">
                Change Plan
              </Link>
            </div>
            {planUsage ? (
              <div className="space-y-3">
                ...
                {planUsage.isAtLimit && (
                  <div className="pt-2">
                    <p className="text-xs text-red-600 font-medium">You've reached your monthly limit.</p>
                    <Link to="/pricing" className="text-xs text-royal font-medium hover:underline">
                      Upgrade to get more sessions
                    </Link>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3">
                ...
                <Link to="/pricing" className="block text-center py-2 bg-royal text-white text-sm font-medium rounded-lg hover:bg-royal-dark">
                  Choose a Plan
                </Link>
              </div>
            )}
